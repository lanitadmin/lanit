...
VALUES (gen_id( hibernate_sequence, 1 ), 'tempTextDocType', 'cardElement=STextTempl', 1, 'Шаблон текста', 0, NULL, 'SDocType', NULL);


INSERT INTO CARD_ELEMENT_FIELD_VALUE(ID, CARD_ELEMENT_ID, FIELD_ID, FIELD_STRING_VALUE)
Select gen_id( hibernate_sequence, 1 ), t.col1, t.col2, t.col3 
	from (
		SELECT  Distinct  
			dc.DOC_TYPE_ID col1, 
			cf.FIELD_ID col2, 
			(select FIRST 1 TEMPLATE_ID from PRINT_DOC_TYPE_TEMPLATE where DOC_TYPE_ID = dc.DOC_TYPE_ID) col3
  		FROM CARD_FIELD cf,
...
INSERT INTO CARD_FIELD (FIELD_ID, FIELD_NAME, FIELD_TYPE, FIELD_VISIBLE, FIELD_CAPTION, FIELD_SIZE, FIELD_IDENT, CARD_CODE, FIELD_REQUIRED)
VALUES (gen_id( hibernate_sequence, 1 ), 'tempTextOgv', 'cardElement=STextTempl', 1, 'Шаблон текста', 0, NULL, 'SGosOrg', NULL);
...
Select gen_id( hibernate_sequence, 1 ), t.col1, t.col2, t.col3 
	from (
		SELECT  Distinct  
			dc.OGV_ID col1, 
			cf.FIELD_ID col2, 
			(select FIRST 1 TEMPLATE_ID from GOSSRVC_OGV_TEMPLATE where OGV_ID = dc.OGV_ID) col3
  		FROM CARD_FIELD cf,
